...
class Utils
{
    ...
    public static function createWallet(): array
    {
        ...
        $privPem = PEM::fromString($privKey);
        ...
        $privKeyLen = strlen($privKeyHex) / 2;
        $pubKeyHex  = bin2hex($ecPrivSeq->at(3)->asTagged()->asExplicit()->asBitString()->string());
        $pubKeyLen  = strlen($pubKeyHex) / 2;

        $pubKeyHex2 = substr($pubKeyHex, 2);
        $pubKeyLen2 = strlen($pubKeyHex2) / 2;

        $hash = Keccak::hash(hex2bin($pubKeyHex2), 256);

        $walletAddress    = '0x' . substr($hash, -40);
        $walletPrivateKey = '0x' . $privKeyHex;
